...
export default function AddSupplierPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      success("Supplier Created", `${formData.name} has been added successfully.`);
      queryClient.invalidateQueries({ queryKey: ["suppliers"] });
      navigate("/dashboard/suppliers");
    },
    onError: (err: any) => {
      error("Error", err.message || "Failed to create supplier.");
    }
  });
    ...
    <div className="min-h-screen bg-muted/30 pb-12 pt-6">
      <div className="max-w-[1600px] mx-auto px-6">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">Onboard a new vendor or individual contractor.</p>
        </div>
        ...
        <Card className="border-border shadow-sm rounded-2xl overflow-hidden">
          ...
          <CardFooter className="p-6 border-t border-border bg-card/50 flex justify-between items-center">
            ...
            {currentStep < totalSteps ? (
              ...
              <Button onClick={handleSubmit} disabled={createMutation.isPending} className="h-11 px-8 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-md shadow-emerald-900/20">
                {createMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving...</> : <><CheckCircle2 className="mr-2 h-5 w-5" /> Finish & Save</>}
              </Button>
            )}
          </CardFooter>
        </Card>
