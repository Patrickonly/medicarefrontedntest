...
export default function AddDiscountRulePage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      success("Success", "Discount rule created successfully");
      queryClient.invalidateQueries({ queryKey: ["admin-discount-rules"] });
      navigate("/dashboard/subscriptions");
    },
    onError: (err: any) => error("Error", err.message || "Failed to create discount rule"),
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate(discountForm);
  };
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Add Discount Rule</h1>
            <p className="text-muted-foreground mt-1">Discounts auto-apply based on subscription duration.</p>
          </div>
        </div>
      </div>
