...
export default function EditOrganizationTypePage() {
  ...
  useEffect(() => {
    if (types.length > 0 && id) {
      const type = types.find((t: OrganizationType) => t.id === id);
      if (type) {
        setFormData({
          name: type.name || "",
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return await api.put(`/api/organization-types?id=${id}`, data);
    },
    onSuccess: () => {
      success("Success", "Organization type updated successfully");
      queryClient.invalidateQueries({ queryKey: ["admin-organization-types"] });
      navigate("/dashboard/organization-types");
    },
    onError: (err: any) => error("Error", err?.message || "Failed to update organization type"),
  });
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Edit Organization Type</h1>
            <p className="text-muted-foreground mt-1">Update the details of this organization type.</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="border-border shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Update the core identity of this organization type.</CardDescription>
          </CardHeader>
