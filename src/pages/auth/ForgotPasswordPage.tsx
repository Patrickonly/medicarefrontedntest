...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!identifier) {
      toastError("Error", "Please enter your email address.");
      return;
    }
    ...
    setLoading(true);
    const { error, userId } = await forgotPassword(identifier);
    
    if (error) {
      setLoading(false);
      toastError("Error", error.message || "Failed to send reset code");
      return;
    }

    setLoading(false);
    success("Success", "Password reset code sent successfully!");
