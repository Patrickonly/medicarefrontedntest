...
export default function EditSubscriptionPlanPage() {
    ...
    if (plans.length > 0 && id) {
      ...
      if (plan) {
        setPlanForm({
          ...
          status: plan.status?.toLowerCase() === "inactive" ? "inactive" : "active",
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: typeof planForm) => {
      return await api.put(`/api/admin/subscriptions/plans?id=${id}`, data);
    },
    onSuccess: () => {
      success("Success", "Plan updated successfully");
      queryClient.invalidateQueries({ queryKey: ["admin-subscription-plans"] });
      navigate("/dashboard/subscriptions");
    },
    onError: (err: any) => error("Error", err.message || "Failed to update plan"),
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMutation.mutate(planForm);
  };
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Edit Subscription Plan</h1>
            <p className="text-muted-foreground mt-1">Update pricing, limits, and included modules for this plan.</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="border-border shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Update the core details and pricing for this subscription plan.</CardDescription>
          </CardHeader>
